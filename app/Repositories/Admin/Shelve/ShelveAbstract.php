...
class ShelveAbstract implements ShelveInterface
{
    ...
    public function getAllProduct($request, int $per_page = 50)
    {
        ...
        try {
        if($request->keywords != ''){
        $dataSet = $this->stock->select('PK_NO', 'SKUID', 'IG_CODE', 'BARCODE', 'PRD_VARINAT_NAME', 'PRD_VARIANT_IMAGE_PATH', 'INV_WAREHOUSE_NAME', 'F_INV_WAREHOUSE_NO','F_PRD_VARIANT_NO')
                    ->whereRaw('(PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420)')
                    ->where('PRD_VARINAT_NAME', 'LIKE', '%' . $request->keywords . '%')
                    ->orWhere('SKUID', 'LIKE', '%' . $request->keywords . '%')
                    ->orWhere('BARCODE', 'LIKE', '%' . $request->keywords . '%')
                    ->orWhere('IG_CODE', 'LIKE', '%' . $request->keywords . '%')
            ...
            if(!empty($dataSet) && count($dataSet)> 0){
                foreach ($dataSet as $k => $value1) {
                    ...
                    $available              = 0;
                    $stock = [];
                    $stock = Stock::select('PK_NO', 'SKUID', 'IG_CODE', 'BARCODE', 'PRD_VARINAT_NAME', 'PRD_VARIANT_IMAGE_PATH', 'INV_WAREHOUSE_NAME', 'F_INV_WAREHOUSE_NO','F_SHIPPMENT_NO', 'F_BOX_NO', 'F_INV_ZONE_NO', 'PRODUCT_STATUS', 'BOOKING_STATUS', 'ORDER_STATUS')->where('IG_CODE',$value1->IG_CODE)->where('F_INV_WAREHOUSE_NO',$value1->F_INV_WAREHOUSE_NO)->whereRaw('(PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420)')->get();

                    if(!empty($stock)){

                        foreach ($stock as $l => $value2) {
                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->BOOKING_STATUS >= 10) && ($value2->BOOKING_STATUS <= 80) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null)){
                                $ordered += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null)){
                                $available += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->F_SHIPPMENT_NO == null) && ($value2->F_BOX_NO != null) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null)){
                                $boxed_qty += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->F_BOX_NO == null || $value2->F_BOX_NO == 0) && ($value2->PRODUCT_STATUS == null || $value2->PRODUCT_STATUS == 0 || $value2->PRODUCT_STATUS == 90 ) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null)){
                                $yet_to_boxed_qty += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->F_SHIPPMENT_NO != null) && ($value2->F_BOX_NO != null) && ($value2->F_INV_ZONE_NO == null) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null) && ($value2->PRODUCT_STATUS < 60)){
                                $shipment_assigned_qty += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->F_INV_ZONE_NO != null) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null)){
                                $shelved_qty += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->F_INV_ZONE_NO == null) && ($value2->ORDER_STATUS < 80 OR $value2->ORDER_STATUS == null) && ($value2->PRODUCT_STATUS >= 60)){
                                $not_shelved_qty += 1;
                            }

                            if( ($value2->IG_CODE == $value1->IG_CODE) && ($value2->F_INV_WAREHOUSE_NO == $value1->F_INV_WAREHOUSE_NO ) && ($value2->ORDER_STATUS >= 80)){
                                $dispatched += 1;
                            }
                        }
                    }
