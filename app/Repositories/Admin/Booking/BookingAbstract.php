...
class BookingAbstract implements BookingInterface
{
    ...
    public function getProductINV($ig_code, $f_booking_no = null,$price_type = null)
    {
        if ($f_booking_no != null) {
            ...
            $count_my_warehouse = Stock::selectRaw('(SELECT IFNULL(COUNT(PK_NO),0) from INV_STOCK where F_INV_WAREHOUSE_NO != 1 and IG_CODE = '. '"' .$ig_code. '"' .' and (BOOKING_STATUS IS NULL OR BOOKING_STATUS = 0 OR BOOKING_STATUS = 90 OR F_BOOKING_NO = '.$f_booking_no.') AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420))')->limit(1)->getQuery();
            ...
            ->whereRaw('( INV_STOCK.BOOKING_STATUS IS NULL OR INV_STOCK.BOOKING_STATUS = 0 OR INV_STOCK.BOOKING_STATUS = 90 OR INV_STOCK.F_BOOKING_NO = '.$f_booking_no.' ) AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420) ')
            ...
            $count_my_warehouse = Stock::selectRaw('(SELECT IFNULL(COUNT(PK_NO),0) from INV_STOCK where F_INV_WAREHOUSE_NO != 1 and IG_CODE = '. '"' .$ig_code. '"' .' and (BOOKING_STATUS IS NULL OR BOOKING_STATUS = 0 OR BOOKING_STATUS = 90) AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420))')->limit(1)->getQuery();
    ...
    public function get_available_qty($ig_code, $qty, $house, $ship, $box, $id, $ship_no)
    {
        $info = Stock::where('IG_CODE', $ig_code)
                    ...
                    $info = $info->whereRaw('( BOOKING_STATUS IS NULL OR BOOKING_STATUS = 0 OR BOOKING_STATUS = 90 ) AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420) ')
                    ->count();
    ...
    public function get_available_qty_update($ig_code, $qty, $house, $ship, $box, $id, $ship_no)
    {
        $info = Stock::where('IG_CODE', $ig_code)
                    ...
                    $info = $info->whereRaw('( BOOKING_STATUS IS NULL OR BOOKING_STATUS = 0 OR BOOKING_STATUS = 90 OR F_BOOKING_NO = '.$id.' ) AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS != 420) ')->count();
                            ...
                            if($bundle && count($bundle) > 0 ){
                                ...
                                if($offer){
                                    foreach ($offer as $key => $value) {
                                        ...
                                        $offer_a = DB::table('SLS_CHECK_OFFER')->where('F_BOOKING_NO',$request->booking_pk_no)->where('IS_TEMP',0)->where('IS_PROCESSED',1)->where('LIST_TYPE','A')->where('F_BUNDLE_NO',$value->F_BUNDLE_NO)->count();
                                        ...
                                        $bundle  = Offer::find($value->F_BUNDLE_NO);
