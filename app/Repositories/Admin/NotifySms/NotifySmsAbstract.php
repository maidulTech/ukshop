...
class NotifySmsAbstract implements NotifySmsInterface
{
    ...
    public function getSendEmail($id){
        try {
            $notis   = EmailNotification::where('IS_SEND',0)->whereNotNull('EMAIL')->where('PK_NO',$id)->first();
            if($notis){
                    $mail_body = $this->getEmailBody($notis->PK_NO);
                    if($notis->TYPE == 'Arrival' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderArrivalEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'Default' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderDefaultEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'Dispatch' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderDispatchEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'Cancel' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderCancelEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'Order Create' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderCreateEndEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'Return' && isset($notis->EMAIL)){
                        $emailRes   = $this->orderReturntEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($notis->TYPE == 'greeting' && isset($notis->EMAIL)){
                        $emailRes   = $this->greetingEmail($mail_body,$notis->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$notis->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }
    ...
    public function getEmailBody($id)
    {
        ...
        if ($data->TYPE == 'Order Create') {
            $data['order_info'] = Booking::select('SLS_BOOKING.DISCOUNT','SLS_ORDER.DELIVERY_NAME','SLS_ORDER.DELIVERY_MOBILE','SLS_ORDER.DELIVERY_ADDRESS_LINE_1','SLS_ORDER.DELIVERY_ADDRESS_LINE_2','SLS_ORDER.DELIVERY_ADDRESS_LINE_3','SLS_ORDER.DELIVERY_ADDRESS_LINE_4','SLS_ORDER.DELIVERY_CITY','SLS_ORDER.DELIVERY_POSTCODE','SLS_ORDER.DELIVERY_STATE','SLS_ORDER.DELIVERY_COUNTRY','SLS_BOOKING.BOOKING_SALES_AGENT_NAME','SLS_BOOKING.RECONFIRM_TIME','SLS_BOOKING.BOOKING_NO','SLS_BOOKING.F_CUSTOMER_NO','SLS_BOOKING.F_RESELLER_NO','SLS_BOOKING.PK_NO')
            ...
            $data['order_info'] = Booking::select('SLS_BOOKING.DISCOUNT','SLS_ORDER.DELIVERY_NAME','SLS_ORDER.DELIVERY_MOBILE','SLS_ORDER.DELIVERY_ADDRESS_LINE_1','SLS_ORDER.DELIVERY_ADDRESS_LINE_2','SLS_ORDER.DELIVERY_ADDRESS_LINE_3','SLS_ORDER.DELIVERY_ADDRESS_LINE_4','SLS_ORDER.DELIVERY_CITY','SLS_ORDER.DELIVERY_POSTCODE','SLS_ORDER.DELIVERY_STATE','SLS_ORDER.DELIVERY_COUNTRY','SLS_BOOKING.BOOKING_SALES_AGENT_NAME','SLS_BOOKING.RECONFIRM_TIME','SLS_BOOKING.BOOKING_NO','SLS_BOOKING.F_CUSTOMER_NO','SLS_BOOKING.F_RESELLER_NO','SLS_ORDER.PK_NO as ORDER_NO')
            ->join('SLS_ORDER','SLS_BOOKING.PK_NO','SLS_ORDER.F_BOOKING_NO')
            ->where('SLS_ORDER.F_BOOKING_NO', $data->F_BOOKING_NO)
            ->first();
            ...
            $data['consignment'] = Dispatch::where('F_ORDER_NO',$data['order_info']->ORDER_NO)->get();
            foreach ($data['stock_info'] as $key => $value) {
                if($value->CURRENT_IS_REGULAR == 1){
                    $value->unit_price = $value->CURRENT_REGULAR_PRICE;
                }else{
    ...
    public function getSendAllEmail($request)
    {
            ...
            if($notis){
                foreach ($notis as $key => $value) {
                    ...
                    if($value->TYPE == 'Arrival' && isset($value->EMAIL)){
                        $emailRes   = $this->orderArrivalEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                        ...
                        $emailRes   = $this->orderDefaultEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($value->TYPE == 'Dispatch' && isset($value->EMAIL)){
                        $emailRes   = $this->orderDispatchEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($value->TYPE == 'Cancel' && isset($value->EMAIL)){
                        $emailRes   = $this->orderCancelEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($value->TYPE == 'Order Create' && isset($value->EMAIL)){
                        $emailRes   = $this->orderCreateEndEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($value->TYPE == 'Return' && isset($value->EMAIL)){
                        $emailRes   = $this->orderReturntEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }elseif($value->TYPE == 'greeting' && isset($value->EMAIL)){
                        $emailRes   = $this->greetingEmail($mail_body,$value->EMAIL);
                        if($emailRes == 1){
                            //EmailNotification::where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
                        }
                    }
        ...
        return $this->formatResponse(true, '', 'admin.notify_email.list',$emailRes);
    }
}
