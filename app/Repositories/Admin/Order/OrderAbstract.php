    ...
    public function ajaxDelete($id,$type,$booking_no)
    {
        ...
        try {
            if($type == 'bundle'){
                ...
                $booking_details    = BookingDetails::select('F_BOOKING_NO','COMISSION','F_INV_STOCK_NO')->where('F_BOOKING_NO',$booking_no)->where('F_BUNDLE_NO',$id)->where('BUNDLE_SEQUENC',$max_bundle_sequenc)->get();
                $total_comm = 0;
                if($booking_details && count($booking_details) > 0 ){
                    foreach($booking_details as $key => $value ){
