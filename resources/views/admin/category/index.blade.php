                ...
                <table class="table table-striped table-bordered alt-pagination table-sm" id="indextable">
                  <thead>
                    <tr>
                      ...
                      {{-- <th class="" style="min-width: 100px;">SKU Prefix</th> --}}
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style="width: 200px;">
                          ...
                          <li class="list-group-item list-group-item-sm list-group-parent">
                            <div class="float-right" style="display: inline-block; min-width: 50px;">
                              <span class="float-right child-action">
                                ...
                                @if(hasAccessAbility('view_hscode', $roles))
                                <button class="btn btn-xs btn-primary mr-0 showSHcode" title="SHOW HS CODE" data-category_id="{{$row->PK_NO}}" data-subcat_id="{{$scat->PK_NO}}"  data-type="edit">HS</button>
                                @endif
                                @if(hasAccessAbility('new_hscode', $roles))
                                <button class="btn btn-xs btn-success mr-0 addSHcode" data-toggle="modal" data-target="#addEditHScode" title="ADD HS CODE" data-url="{{ route('admin.hscode.store') }}" data-subcat_id="{{$scat->PK_NO}}" data-subcat_name="{{$scat->NAME}}" data-type="add">+ HS</button>
                                @endif
                              ...
                              <span> {{$key+1}} . </span>
                              <span title="Size name">{{$scat->NAME}}</span> &nbsp;
                                  ...
                                  <ul class="list-group ">
                                    ...
                                    <li class="list-group-item list-group-item-sm list-group-parent">
                                      <span class=" float-right child-action">
                                        ...
                                        @if(hasAccessAbility('view_hscode', $roles))
                                        <button class="btn btn-xs btn-success mr-0 showSHcode" title="SHOW HS CODE" data-category_id="{{$row->PK_NO}}" data-subcat_id="{{$scat->PK_NO}}"  data-type="edit">HS</button>
                                        @endif

                                        @if(hasAccessAbility('new_hscode', $roles))
                                        <button class="btn btn-xs btn-primary mr-0 addSHcode" data-toggle="modal" data-target="#addEditHScode" title="ADD HS CODE" data-url="{{ route('admin.hscode.store') }}" data-subcat_id="{{$scat->PK_NO}}" data-subcat_name="{{$scat->NAME}}" data-type="add">+ HS</button>
                                        @endif

                                        @if(hasAccessAbility('edit_hscode', $roles))
                                        <button class="btn btn-xs btn-info mr-0 editSubcategory" data-toggle="modal" data-target="#addEditSubcategory" title="EDIT SUBCATEGORY" data-url="{{ route('admin.sub_category.update', [$scat->PK_NO]) }}" data-category_id="{{$row->PK_NO}}" data-category_name="{{$row->NAME}}" data-subcat_id="{{$scat->PK_NO}}" data-subcat_name="{{$scat->NAME}}" data-subcat_code="{{$scat->CODE}}" data-type="edit"><i class="la la-edit"></i></button>
                                        @endif
                                      ...
                                      <span> {{$key+1}} . </span>
                                      <span title="Model name">{{$scat->NAME}}</span> &nbsp;
                                      {{--<span title="Model code">({{$scat->CODE}})</span> --}}
                                    </li>
                                    @endif
                                    @endforeach()
                        ...
                        <td>
                          @if($row->allHScodes && $row->allHScodes->count() > 0 )
                          <ul class="list-group" >
                            @foreach($row->allHScodes as $key => $hscode)

                            <li class="list-group-item list-group-item-sm list-group-parent hs_code_list hs_cat_{{$row->PK_NO}} hs_scat_{{$hscode->SCAT_PK_NO}}" style="margin: 2px;">
                              <div class="float-right" style="display: inline-block; min-width: 50px;">
                                <span class="float-right child-action">
                                  @if(hasAccessAbility('edit_hscode', $roles))
                                  <button class="btn btn-xs btn-primary mr-0 editSHcode" data-toggle="modal" data-target="#addEditHScode" title="EDIT HS CODE" data-url="{{ route('admin.hscode.update', [$hscode->HS_PK_NO]) }}" data-subcat_id="{{$hscode->SCAT_PK_NO}}" data-subcat_name="{{$hscode->SCAT_NAME}}" data-hscode="{{$hscode->HS_CODE}}" data-narration="{{$hscode->HS_NARRATION}}" data-type="edit"><i class="la la-edit"></i></button>
                                  @endif

                                  @if(hasAccessAbility('delete_hscode', $roles))
                                    <a href="{{ route('admin.hscode.delete', [$hscode->HS_PK_NO]) }}" class="btn btn-xs btn-danger" title="DELETE HS CODE" onclick="return confirm('Are you sure you want to delete?')"><i class="la la-trash"></i>
                                    </a>
                                  @endif
                                ...
                                <span title="HS CODE - {{$hscode->SCAT_NAME}}">{{$hscode->HS_CODE}}</span> &nbsp;
                                @if($hscode->HS_NARRATION)
                                <span title="HS CODE NARRATION"> - {{$hscode->HS_NARRATION}}</span>
                                @endif
